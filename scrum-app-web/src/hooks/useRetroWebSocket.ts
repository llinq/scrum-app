...
export function useRetroWebSocket(boardId: string | null, events: WebSocketEvents = {}) {
  ...
  useEffect(() => {
    ...
    if (!boardId || currentBoardIdRef.current === boardId) {
      console.log('Skipping connection - no boardId or already connected to this board');
      return;
    }
    ...
    socket.on('columns-reordered', ({ columns }: { columns: RetroColumn[] }) => {
