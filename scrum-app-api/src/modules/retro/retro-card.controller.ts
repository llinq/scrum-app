...
export class RetroCardController {
  ...
  @ApiResponse({ status: 201, description: 'Card criado com sucesso.', type: RetroCardResponseDto })
  @ApiResponse({ status: 400, description: 'Dados inválidos ou cards anônimos não permitidos.' })
  ...
  async create(
    ...
    return this.cardService.create(columnId, createDto, userId);
  }

  @Get('column/:columnId')
  @ApiOperation({ summary: 'Listar cards de uma coluna' })
  @ApiParam({ name: 'columnId', description: 'ID da coluna' })
  @ApiResponse({ status: 200, description: 'Lista de cards retornada com sucesso.', type: [RetroCardResponseDto] })
  @ApiResponse({ status: 404, description: 'Coluna não encontrada.' })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  async findByColumnId(@Param('columnId') columnId: string) {
    return this.cardService.findByColumnId(columnId);
  }
  ...
  @ApiResponse({ status: 200, description: 'Card encontrado com sucesso.', type: RetroCardResponseDto })
  ...
  @ApiParam({ name: 'id', description: 'ID do card' })
  @ApiResponse({ status: 200, description: 'Card atualizado com sucesso.', type: RetroCardResponseDto })
  @ApiResponse({ status: 400, description: 'Dados inválidos.' })
  @ApiResponse({ status: 403, description: 'Acesso negado. Apenas o autor ou criador do board pode editar.' })
  @ApiResponse({ status: 404, description: 'Card não encontrado.' })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  async update(
    @Param('id') id: string,
  ...
  @ApiParam({ name: 'id', description: 'ID do card' })
  @ApiResponse({ status: 200, description: 'Card deletado com sucesso.', type: MessageResponseDto })
  @ApiResponse({ status: 403, description: 'Acesso negado. Apenas o autor ou criador do board pode deletar.' })
  @ApiResponse({ status: 404, description: 'Card não encontrado.' })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  async delete(
    @Param('id') id: string,
  ...
  @ApiParam({ name: 'id', description: 'ID do card' })
  @ApiResponse({ status: 201, description: 'Voto adicionado com sucesso.', type: MessageResponseDto })
  @ApiResponse({ status: 400, description: 'Limite de votos atingido, votação desabilitada ou já votou neste card.' })
  @ApiResponse({ status: 404, description: 'Card não encontrado.' })
  @ApiResponse({ status: 401, description: 'Não autorizado.' })
  async addVote(
  ...
  @ApiParam({ name: 'id', description: 'ID do card' })
  @ApiResponse({ status: 200, description: 'Voto removido com sucesso.', type: MessageResponseDto })
